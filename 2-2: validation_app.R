...
labels_file  <- '/Users/katrine/Desktop/HAWKEARS_Labels6(A1)-Stripped.csv'
audio_dir    <- "/Volumes/BUworkspace/BayneLabWorkSpace/Katrine_workspace/AudioMothSync/OKLG-6-Sync/"
results_file <- "/Users/katrine/Desktop/HawkEars_validation_results.csv"
...
make_snippet <- function(clip_row) {
  # We search for the filename inside your audio_dir and all subfolders
  all_matches <- list.files(
    path = audio_dir, 
    pattern = clip_row$filename, 
    recursive = TRUE, 
    full.names = TRUE
  )
  
  if (length(all_matches) == 0) {
    # If the first search fails, we try a broader search just in case
    warning("File not found: ", clip_row$filename)
    return(NA_character_)
  }
  
  # Take the first match found
  wav_path <- all_matches[1]
